...
export default function WatchContent() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        if (data) {
          setContent(data);
          const relatedData = await fetchMedia(1, 10, undefined, data.category);
          setRelated(relatedData.data?.filter((m: MediaItem) => m.id !== data.id) || []);
        }
      } catch (error) {
  ...
  if (!content) {
    return (
      <main className="flex min-h-screen items-center justify-center text-white">
        <div className="text-center">
          <h1 className="mt-4 text-2xl font-bold">Content not found</h1>
        </div>
      </main>
    );
  }
